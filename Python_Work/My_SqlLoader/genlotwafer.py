...
def GenLotWafer():
    # 从psmc_wip_traceing_table数据中获取当前时间=出货时间的lot信息，并返回元祖
    mysql = MySQL()
    connection = MySQLdb.connect(**mysql.testdb_config)
    with connection.cursor() as cursor:
        cursor.execute("""SELECT DISTINCT MLot_ID FROM psmc_wip_tracing_table""")
        wip_lot = cursor.fetchall()
        cursor.execute("""SELECT DISTINCT MLot_ID FROM psmc_lot_wafer""")
        wafer_lot = cursor.fetchall()
    connection.close()
    df_wip_lot = [i[0] for i in wip_lot]
    df_wafer_lot = [i[0] for i in wafer_lot]
    list2 = list(set(df_wip_lot) - set(df_wafer_lot))
    new_df = dict()
    for i in list2:
        new_df['Mlot_ID'] = i
        new_df['Wafer_No'] = ['#01', '#02', '#03', '#04', '#05', '#06', '#07', '#08', '#09', '#10',
