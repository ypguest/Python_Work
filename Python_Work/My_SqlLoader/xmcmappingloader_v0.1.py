...
def mappingloader(_file_path):
    ...
    rename = {'XMC LOT': 'XMC_LOT_ID', 'XMC WAFER': 'XMC_WAFER_NO', 'PTC LOT': 'PSMC_LOT_ID', 'PTC WAFER': 'PSMC_WAFER_NO', 'SMIC LOT': 'SMIC_LOT_ID', 'SMIC WAFER': 'SMIC_WAFER_NO'}
    ...
    for file_path in file_paths:
        ...
        try:
            ...
            continue
        table.rename(columns=lambda x: x.upper(), inplace=True)
        ...
        try:
            table['SMIC LOT'] = table['SMIC LOT'].str[:6]
            table['SMIC WAFER'] = table['SMIC WAFER'].apply(lambda x: '#' + ('0' + str(x))[-2:])
        except KeyError:
            pass
