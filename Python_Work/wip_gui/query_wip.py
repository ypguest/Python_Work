                            ...
                            for result in sql_results:
                                if result[4] == '1':   # 选取Wafer No为1的bin
                                    for binNo in result[3].split(','):
                                        ...
                                        # print(str(data['Current_Chip_Name'].values[-1]), mlot, mlot, row.name, row.name)
                        # connection = pymysql.connect(**sql_config)
                        # with connection.cursor() as cursor:
                        #     # noinspection PyBroadException
                        #     try:
                        #         cursor.execute('USE testdb;')
                        #         cursor.execute("INSERT INTO psmc_lot_wafer(Current_Chip_Name, Mlot_ID, Lot_Wafer_No, Wafer_No)"
                        #                        "VALUES ('%s','%s','%s%s','%s')" % (str(data['Current_Chip_Name'].values[-1]), mlot, mlot, row.name, row.name))
                        #         connection.commit()
                        #         cursor.close()
                        #         connection.close()
                        #     except Exception:
